...
<?php include($_SERVER["DOCUMENT_ROOT"]."/Admin/left.php") ?>
 ...
 <?php
        ...
        if(isset($_GET["net"])){
        $dtypetoedit = trim(htmlspecialchars($_GET["net"]));
         //echo $dplantoedit;
         
        //check if the api exist
        $q = $conn->prepare("SELECT * FROM data_type WHERE Index_Id=:em");
        $q->bindParam(':em', $dtypetoedit, PDO::PARAM_STR);
        $q->execute();
        if($q->rowCount() > 0){
...
<form action="/Controller/Admin/Data/DatatypeeditController.php" method="POST" enctype="multipart/form-data" >
    ...
    <input type="text" name="typ" class="input" value="<?php echo $typ; ?>"/><br/></P><hr/>
...
<button type="submit" value="datatype" name="data_typeedit" class="regbtn">Submit</button>
</form>
</fieldset>
    </div>
