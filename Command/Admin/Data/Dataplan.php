...
Class Dataplan{
    public function userDataplan($net, $plan, $dis, $api, $sta){
        try{
            global $dataplanTable;
            global $dataplanTableId;
            global $conn;

            if($net !="" && $plan !="" && $dis !="" && $api !="" && $sta !=""){

                $qq = $conn->prepare("INSERT INTO $dataplanTable(
                Network, Plan, Discount, Api, Stat) VALUES(:nt, :pl, :ds, :ap, :st )");
                $qq->bindParam(':nt', $net, PDO::PARAM_STR);
                $qq->bindParam(':pl', $plan, PDO::PARAM_STR);
                ...
                ?>  
                <script>
                window.location='/Admin/data/data-planf.php? error'; 
                </script> 
                <?php 
                exit();");
